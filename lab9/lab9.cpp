...
int main() {
  ...
  printf("%s %9s %6s %4s %5s %2s %8s %6s\n", header.otherF.f4, "|",
         header.otherF.f5, "|", header.f2, "|", header.f3, "|");
  printf("-----------------------------|----------|---------------|\n");
  printf("%-17s%s%-11s%s    %-6c%s      %-9d%s\n", b->bookname, "|", b->author,
         "|", b->availibility, "|", b->price, "|");
  printf("-----------------------------|----------|---------------|\n");
  printf("%-17s%s%-11s%s    %-6c%s      %-9d%s\n", (b + 1)->bookname, "|",
         (b + 1)->author, "|", (b + 1)->availibility, "|", (b + 1)->price, "|");
  printf("-----------------------------|----------|---------------|\n");
  printf("%-17s%s%-11s%s    %-6c%s      %-9d%s\n", (b + 2)->bookname, "|",
         (b + 2)->author, "|", (b + 2)->availibility, "|", (b + 2)->price, "|");
  printf("-----------------------------|----------|---------------|\n");
  printf("%-17s%s%-11s%s    %-6c%s      %-9d%s\n", (b + 3)->bookname, "|",
         (b + 3)->author, "|", (b + 3)->availibility, "|", (b + 3)->price, "|");
  printf("-----------------------------|----------|---------------|\n");
  printf("%-17s%s%-11s%s    %-6c%s      %-9d%s\n", (b + 4)->bookname, "|",
         (b + 4)->author, "|", (b + 4)->availibility, "|", (b + 4)->price, "|");
  printf("---------------------------------------------------------\n");
...
void changeRows(struct Books *source, int sourceSize) {
  ...
  printf("Выберете номер первой строки для проведения операции замены (от 1 до "
         "5): ");
  while (1) {
  ...
  printf("Выберете номер второй строки для проведения операции замены (от 1 до "
         "5): ");
  while (1) {
    if (fgets(input, sizeof(input), stdin) != NULL) {
      input[strcspn(input, "\n")] = 0;
